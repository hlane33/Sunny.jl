...
function generate_two_particle_basis(cluster_size::NTuple{3, Int}, numbands::Int)
    ...
    for ci in cartes_indices, cj in cartes_indices
        ...
        j = linear_indices[cj]
        if i ≤ j
            q1_index = linear_qs_indices[ci[1], ci[2], ci[3]]
            q2_index = linear_qs_indices[cj[1], cj[2], cj[3]]
            q1 = qs[q1_index]
            q2 = qs[q2_index]
            qcom = mod.(q1+q2, 1.0)
            qcom_carts_index = findfirst(x -> x ≈ qcom, qs)
            qcom_index = linear_qs_indices[qcom_carts_index]
...
struct RealSpaceQuarticVertices
    ...
    V42 :: Array{ComplexF64, 3}
    V43 :: Array{ComplexF64, 3}
end
...
struct NonPerturbativeTheory
    ...
    Es :: Array{Vector{Float64}, 3}
    Vs :: Array{Matrix{ComplexF64}, 3}
...
function calculate_real_space_quartic_vertices(swt::SpinWaveTheory)
    ...
    V42_buf = zeros(ComplexF64, N-1, N-1, N-1)
    V43_buf = zeros(ComplexF64, N-1, N-1, N-1)
        ...
        for coupling in int.pair
            ...
            for (A, B) in coupling.general.data
                for σ1 in 1:N-1, σ2 in 1:N-1, σ3 in 1:N-1
                    V42_buf[σ1, σ2, σ3] += -0.5 * A[N, σ1] * B[N, σ3]
                    V43_buf[σ1, σ2, σ3] += -0.5 * A[σ1, N] * B[N, σ3]
                    for σ4 in 1:N-1
...
function NonPerturbativeTheory(swt::SpinWaveTheory, clustersize::NTuple{3, Int})
    ...
    Es = [zeros(L) for _ in 1:Nu1, _ in 1:Nu2, _ in 1:Nu3]
    Vs = [zeros(ComplexF64, 2L, 2L) for _ in 1:Nu1, _ in 1:Nu2, _ in 1:Nu3]
    ...
    for (iq, q) in enumerate(qs)
        ...
        Es[iq] = E
        Vs[iq] = deepcopy(V_buf)
    end

    real_space_quartic_vertices = calculate_real_space_quartic_vertices(swt)

    return NonPerturbativeTheory(swt, clustersize, two_particle_states, Es, Vs, real_space_quartic_vertices)
